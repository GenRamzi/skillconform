...
async function main(): Promise<void> {
  try {
    ...
    if (parsed.command === "inventory") {
      if (parsed.options.format === "sarif") throw new Error("The inventory command supports pretty or json output.");
      const inventory = buildCapabilityInventory(parsed.target);
      writeReport(renderCapabilityInventory(inventory, parsed.options.format), parsed.options.output);
      return;
    }
...
The open-source CI and test suite for Agent Skills.
